...
(async function query_codes() {
    try {
        var codes = await d3.json("/bridge/stockcodes");
        
        stock_codes = codes.map(function(d) {
            return {
                code: d.code,
                name: d.name
            };
...
async function predict(code) {
    //data = await d3.json("/bridge/predict?code=" + code + "&data_from=20190818&data_to=20200819")
    data = await d3.json("/bridge/predict?code=" + code + "&n=250")
    console.log(data)
    ...
    action=[]
    stock_data.trades = data.map(function(d, index){
        // stock_data.trades = [
        ...
        {
            if (d.action == 0) {
                strAction = 'stock_data.ohlc[' + index + '].date, type: \'buy\', price: stock_data.ohlc[' + index + '].low, low: stock_data.ohlc['+ index + '], high: stock_data.ohlc['+ index +'].high'
                action.append(strAction)
            } else if (d.action == 1) {
                strAction = 'stock_data.ohlc[' + index + '].date, type: \'sell\', price: stock_data.ohlc[' + index + '].low, low: stock_data.ohlc['+ index + '], high: stock_data.ohlc['+ index +'].high'
                action.append(strAction)
            } 
        }
        return action
...
async function query_candle(code) {
    data = await d3.json("/bridge/stockcandles?code=" + code + "&n=250")
    //console.log(data)
    stock_data.ohlc = data.map(function(d) {
        //console.log(d);
        return {
...
function draw(selection) {
    ...
    data = stock_data.ohlc;
    //console.log(data)
    
    var accessor = candlestick.accessor(),
        indicatorPreRoll = stock_data.preroll,
    ...
    svg.select("text.symbol").text(stock_data.name);
    ...
    // svg.select("g.trendlines").datum(stock_data.trendlines).call(trendline).call(trendline.drag);
    // svg.select("g.supstances").datum(stock_data.supstances).call(supstance).call(supstance.drag);
    svg.select("g.tradearrow").datum(stock_data.trades).call(tradearrow);
...
// function updateChart(new_stock) {
